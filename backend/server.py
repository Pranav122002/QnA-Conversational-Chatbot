...
def identify_context(question, chunks):
    question_keywords = set([word.lower() for word in word_tokenize(question) if word.isalnum() and word.lower() not in stop_words])
    ...
    for context in chunks:
        ...
        context_keywords = set([word.lower() for word in word_tokenize(context) if word.isalnum() and word.lower() not in stop_words])
